...
const uploadImagesBlog = asyncHandler(async (req, res) => {
    ...
    // $push là đẩy dữ liệu vào - moogose
    const response = await Blog.findByIdAndUpdate(bid, { $push: { image: req.file.path } }, { new: true })
    return res.status(200).json({
        status: response ? true : false,
        updatedProduct: response ? response : 'Cannot upload images blog'
    })
})
