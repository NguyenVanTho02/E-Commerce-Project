...
const register = asyncHandler(async (req, res) => {
    ...
    if (user) throw new Error('User has existed')
        ...
        if (newUser) {
            ...
            await sendMail({ email, html, subject: 'Config register account in Digital World' })
        }
        setTimeout(async () => {
        ...
        return res.json({
            success: newUser ? true : false,
            mess: newUser ? 'Please check your email to active account' : 'Some went wrong, please try later'
        })
    }
})
...
const finalRegister = asyncHandler(async (req, res) => {
    ...
    return res.json({
        ...
        mess: notActivedEmail ? 'Register is Successfully. Please go login' : 'Some went wrong, please try later'
    })
})
...
const forgotPassword = asyncHandler(async (req, res) => {
    ...
    return res.status(200).json({
        ...
        mess: rs.response?.includes('OK') ? 'Check your mail please.' : 'Something went wrong. Please try later'
    })
})
...
const updateUser = asyncHandler(async (req, res) => {
    ...
    // Loại bỏ những trường không lấy về
